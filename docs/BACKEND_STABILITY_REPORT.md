...
| Bug                                                         | Root Cause                                                                                                                                  | Fix                                                                                                                                                    |
|-------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------|
| Timeout never firing / cancel-rerun double Gemini           | Historical: wait_for cancelled in-flight turn then background re-ran pipeline                                                               | Now: `asyncio.wait` race on `_session_turn_locked` (`WHATSAPP_WEBHOOK_TIMEOUT` default 12s); slow path `_await_inflight_and_push` (no cancel)             |
